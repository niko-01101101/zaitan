...
static NSRect lerpRect(NSRect a, NSRect b, CGFloat t) {
  return NSMakeRect(a.origin.x + (b.origin.x - a.origin.x) * t,
...
- (void)updatePlacements:(const std::vector<WindowPlacement> &)placements
  ...
  for (auto &p : placements) {
    ...
    if (it == _panes.end()) {
      // Position snaps immediately; size lerps in from 90% centered on the
      // target.
      CGFloat dw = target.size.width * 0.2;
      CGFloat dh = target.size.height * 0.2;
      NSRect initial =
          NSMakeRect(target.origin.x + dw / 2, target.origin.y + dh / 2,
                     target.size.width * 0.9, target.size.height * 0.9);
      _panes[p.windowID] = {initial, target};
    } else {
  ...
  if (focusedID == 0 || !incoming.count(focusedID))
    _hasCursor = NO;
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  for (auto &kv : _panes) {
    NSBezierPath *path =
        [NSBezierPath bezierPathWithRoundedRect:kv.second.current
                                        xRadius:8
                                        yRadius:8];
    path.lineWidth = 1.5;
  ...
  if (_hasCursor) {
    NSBezierPath *path = [NSBezierPath bezierPathWithRoundedRect:_cursor.current
                                                         xRadius:8
                                                         yRadius:8];
    path.lineWidth = 2.0;
...
void showDebugOverlay(const std::vector<WindowPlacement> &placements,
  ...
  [gOverlayView updatePlacements:placements
