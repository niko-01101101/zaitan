...
Node *Desktop::findByWindowID(Node *node, uint32_t windowID) {
    ...
    if (node->isLeaf()) return node->pane->windowID == windowID ? node : nullptr;
...
Node *Desktop::findEmptyPane(Node *node) {
    if (!node) return nullptr;
    if (node->isLeaf()) return node->pane->windowID == 0 ? node : nullptr;
...
void Desktop::reframe(Node *node, WMRect frame) {
    node->frame = frame;
    if (node->isLeaf()) { node->pane->frame = frame; return; }
...
bool Desktop::splitNode(Node *node, SplitDirection dir, uint32_t newWindowID) {
    if (!node || !node->isLeaf() || paneCount >= MAX_PANES) return false;
...
void Desktop::collectLayout(Node *node, std::vector<WindowPlacement> &out) const {
    if (!node) return;
    if (node->isLeaf()) {
...
bool Desktop::splitHorizontally(uint32_t windowID, uint32_t newWindowID) {
    return splitNode(findByWindowID(root.get(), windowID), SplitDirection::Horizontal, newWindowID);
}

bool Desktop::splitVertically(uint32_t windowID, uint32_t newWindowID) {
    return splitNode(findByWindowID(root.get(), windowID), SplitDirection::Vertical, newWindowID);
}
...
bool Desktop::removeWindow(uint32_t windowID) {
    ...
    std::unique_ptr<Node> sibling = std::move(nodeIsLeft ? parent->right : parent->left);
    ...
    if (parent->parent) {
        ...
        (parentIsLeft ? parent->parent->left : parent->parent->right) = std::move(sibling);
    } else {
